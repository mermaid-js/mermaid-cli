...
const zenumlIIFEPath = path.resolve(
  path.dirname(
...
async function cli() {
  ...
  commander
    ...
    .option(
      "--icons <packages...>",
      'Icon packages to use (e.g. "logos fa-solid fa-regular")',
      []
    )
    .parse(process.argv);
  ...
  // Strip @iconify-json/ prefix if provided
  const iconPackages = icons
    .flatMap(
      /**
       * @param {string} i
       * @returns {string[]}
 ...
 * @property {string[]} [iconPackages] - Icon packages to use.
 */
...
async function run(
  input,
  output,
  { puppeteerConfig = {}, quiet = false, outputFormat, parseMMDOptions } = {}
) {
  /**
